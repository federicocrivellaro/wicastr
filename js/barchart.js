  ...
  for (var i = 0; i < res.length; i++) {
    ...
    if(isAvailableData(entity.spam)){
      if(entity.spam===false){
        ...
        addBarchartValue(parameter,totalObj);
      }
    }
  }
...
function addBarchartValue(parameter,totalObj){
...
function drawBarChart(id,totalObj){
  ...
  d3.select(id).append('ul').attr('class', 'legendBarchart col-xs-12').selectAll('li')
  .data(legend)
  .enter().append('li')
