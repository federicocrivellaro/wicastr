    ...
    for (var i = 0; i < res.length; i++) {
        if (checkSourceAvailable(res[i].coordinates)) {
            ...
            if (checkSourceText(res[i].map_text)) {
                ...
                locArrays.descriptions.push("");
            }

            // FIELD SPECIFIC
            var pictures = ""; // LIST OF IMAGES
            ...
            for (var x = 0; x < res[i].fields.length; x++) {
                ...
                } else if (res[i].fields[x].id == "580879c40a975a6aca3feba1") {
                    locArrays.title.push(Autolinker.link(checkSourceText(res[i].fields[x].value)));
                    ...
                    switch (id) {
                        ...
                        case 11:
                            if (checkSourceAvailable(res[i].fields[x].value)) {
                                ...
                                // BARCHART STAST FILL ARRAY
                                if(res[i].fields[x].id == "580879c40a975a6aca3feba7") {
                                    for (var z = 0; z < res[i].fields[x].value.length; z++) {
                                        addBarchartValue(res[i].fields[x].value[z],barChartInitiative);
                                    }
                                }
            ...
            if (res[i].comments != null) {
                ...
                locArrays.comments.push(""); // PUSH REPORT ARRAY OF IMAGES
            }

            

            $('#counter').text(partecipantsCounter);
        } // SKIP IF THERE ARE NOT COORDINATES
    }
    drawBarChart('#barChartInitiative',barChartInitiative);
...
function createClusters(locArrays, i) {
    if (locArrays.locations.length == (i + 1)) {
        var clusterStyles = [{
            textColor: 'black',
            url: assetsRoot + 'assets/imgs/cluster/m1.png',
            height: 50,
            width: 51,
        }, {
            textColor: 'black',
            url: assetsRoot + 'assets/imgs/cluster/m2.png',
            height: 50,
            width: 51,
        }, {
            textColor: 'black',
            url: assetsRoot + 'assets/imgs/cluster/m3.png',
            height: 50,
            width: 51,
        }, {
            textColor: 'black',
            url: assetsRoot + 'assets/imgs/cluster/m4.png',
            height: 50,
            width: 51,
        }, {
            textColor: 'black',
            url: assetsRoot + 'assets/imgs/cluster/m5.png',
            height: 50,
            width: 51,
        }];
...
function buildInfo(locArrays, i) {
    ...
    contentString = contentString +
        ...
        '</div>'+  */
        '<div class="info">' +
        '<p class="name">' + locArrays.title[i] + '</p>' +
        '<p class="date">' + locArrays.when[i] + '</p>' +
        '</div>' +
        '</div>' +
